...
class MuseGAN():
    ...
    def train_critic(self, x_train, batch_size, using_generator):
        ...
        if using_generator:
            ...
            idx = np.random.randint(0, x_train.shape[0],batch_size)#(0, x_train.shape[0], batch_size) #
    ...
    def sample_images(self, run_folder):
        ...
        n_epoch = str(self.epoch)
        #np.save(os.path.join(run_folder, 'images',"sample_%d.npy" % self.epoch), gen_scores)
        np.save('{}/images/{}'.format(run_folder,'sample_'+n_epoch+'.npy' ),gen_scores)
    ...
    def notes_to_score_0(self, midi_note_score, parts, max_pitches, tracks=0):
        last_x = int(midi_note_score[:, tracks][0])
        s = stream.Part()
        dur = 0
        i = tracks
    ...
    def notes_to_score_1(self, midi_note_score, parts, max_pitches, tracks=1):
        last_x = int(midi_note_score[:, tracks][0])
        s = stream.Part()
        dur = 0
        i = tracks
    ...
    def save(self, folder):
                        #os.path.join(folder,'params.pkl')
